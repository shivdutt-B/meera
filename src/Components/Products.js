...
function Products(props) {
  ...
  const [currentCategory, setCurrentCategory] = useState("som val")
  ...
  async function fetchByCategory() {
    ...
    if (ContextItems.categoryData[category]) {
      console.log('we already have the products no need to fetch them')
    }
    else {
      console.log('its his first so we need to filter products')
      ...
      // await setData(fetch.data.products)
